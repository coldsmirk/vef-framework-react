  ...
  return {
    ...
    onNodesChange: changes => {
      ...
      set(state => {
        ...
        if (state.selectedNodeId && state.nodes.every(node => node.id !== state.selectedNodeId)) {
    ...
    removeEdge: edgeId => {
      ...
      if (readonly || edges.every(edge => edge.id !== edgeId)) {
        return;
      }
    ...
    updateNodeData: (nodeId, data) => {
      ...
      if (readonly || nodes.every(n => n.id !== nodeId)) {
        return;
      }
