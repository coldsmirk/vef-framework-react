/* eslint-disable require-await -- test fixtures are intentionally declared `async` to exercise async-function detection without awaiting */
...
// eslint-disable-next-line @typescript-eslint/no-empty-function -- intentionally empty: a function whose implicit return value is `undefined`
...
describe("utils/function", () => {
  describe("isAsyncFunction", () => {
    ...
    it("handles function constructors", () => {
      // eslint-disable-next-line @typescript-eslint/no-empty-function -- only the constructor reference is needed to derive the AsyncFunction constructor
      const AsyncFunctionConstructor = async function () {}.constructor;
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment -- the dynamic Function constructor signature is not statically typed
      // @ts-expect-error
      const asyncFunctionFromConstructor = new AsyncFunctionConstructor("return 'test'");

      // eslint-disable-next-line @typescript-eslint/no-empty-function -- only the constructor reference is needed to derive the Function constructor
