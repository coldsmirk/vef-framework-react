...
import type { LoginParams, LoginResult, ResolveChallengeParams, UserInfo } from "@vef-framework-react/starter";
...
export const resolveChallenge = apiClient.createMutationFn(
  "resolve_challenge",
  ({ post }) => async (params: ResolveChallengeParams) => {
    const { message, data: loginResult } = await post<Except<LoginResult, "message">>(
      API_PATH,
      { data: createApiRequest("security/auth", "resolve_challenge", params) }
    );
    return { message, ...loginResult };
  }
);
...
export const getUserInfo = apiClient.createQueryFn(
  "get_user_info",
  ({ post }) => async ({ appId }: { appId: string }) => {
    const result = await post<UserInfo>(
      API_PATH,
      { data: createApiRequest("security/auth", "get_user_info", { appId }) }
    );
    return result.data;
  }
);
